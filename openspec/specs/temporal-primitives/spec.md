...
A `placement` is a concrete `start` datetime with timezone and a DURATION, and SHALL exist only on a resolved intention or a COMMITMENT. It is the single point where this specification touches clock time, and it is written only as the result of a RESOLUTION or an import.
